...
- **Синхронизация недоступных периодов с 1С не работает** — логов не было, теперь есть (Phase 6.14). Причина сбоя всё ещё не диагностирована — предоставленный пользователем `.pfx`-сертификат оказался НЕ про эту интеграцию (см. Phase 6.15/6.17), реальный URL источника прописан и сохранён. Ждём вывод логов бэкенда после следующего запуска синка со вкладки «Интеграции».

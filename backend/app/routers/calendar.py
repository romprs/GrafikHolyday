...
from app.services import blocked_period_service, delegation_service, leave_request_service
...
def get_blocked_ranges(
    ...
    subject = delegation_service.resolve_subject(db, user, on_behalf_of)
    resolved_from = date_from or date.today()
    resolved_to = date_to or (resolved_from + timedelta(days=365))
    blocks = blocked_period_service.get_effective_blocks(db, subject, resolved_from, resolved_to)
    return [
